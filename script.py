...
def copy_file():
	read_file = pd.read_csv (r'sample.csv')
	read_file.to_excel (r'Bill File Updated.xlsx', index = None, header=True)

bill_file_location = "Bill_File.xlsx"
location = "sample.csv"
file = None
thewriter=None
...
def append_file(col1,col2,col3, col4,col5,col6,col7,col8,col9,col10,col11):
	wb = op.load_workbook('demo.xlsx')
...
def k_app_file(k,n):
	outWorkbook = xlsxwriter.Workbook(k_app)
	outSheet = outWorkbook.add_worksheet()

	field_names = ["Last K Number", "Last app number"]
...
if os.path.exists(k_app)==False:
	...
	datak = pd.read_excel(r'script/k_and_app.xlsx') 
	dfk = pd.DataFrame(datak)
	app_no = int(dfk.iloc[0]['Last app number']) + 1
...
def apps(ind):
	...
	if ind<=90:
		...
		return '(//android.widget.ImageView[@content-desc="PayZapp+"])[' + str(x+3) + ']'
...
while k<length:
	...
	try:
		...
		file = open(location, 'a', newline='')
		thewriter = csv.DictWriter(file, fieldnames=fieldnames)
		thewriter.writerow({'Due Date':df.iloc[k]['Due Date'],	'Description':df.iloc[k]['Description'], 'Biller Name':df.iloc[k]['Biller Name'], 'K Number':df.iloc[k]['K Number'],'Amount':df.iloc[k]['Amount'],	'Status': "Recheck", 'Reason':"Unable to fetch transaction details", 'Biller name on bill':lis[1], 'Amount on bill':lis[2], 'Reference no.':"",'App no.':app_no})
		file.close()
